...
{
    public System.Collections.ObjectModel.ObservableCollection<Client.Manager.Models.PolicyGroup> Groups { get; } = new();

    public PoliciesViewModel()
    {
        Groups.Add(new Client.Manager.Models.PolicyGroup { Name = "Отдел разработки", Type = "Отдел", Members = 8, Template = "IT-стандарт", Inherited = true });
        Groups.Add(new Client.Manager.Models.PolicyGroup { Name = "Бухгалтерия", Type = "Отдел", Members = 5, Template = "Финансы", Inherited = true });
        Groups.Add(new Client.Manager.Models.PolicyGroup { Name = "Проект «Альфа»", Type = "Проект", Members = 12, Template = "Базовый", Inherited = false });
        Groups.Add(new Client.Manager.Models.PolicyGroup { Name = "Контрагенты", Type = "Внешние", Members = 3, Template = "Гостевой", Inherited = false });
    }
}